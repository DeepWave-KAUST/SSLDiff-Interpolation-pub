...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            self.run_step(batch_shot_near, batch_shot_far, cond)
    ...
    def run_step(self, batch_shot_near, batch_shot_far, cond):
        self.forward_backward(batch_shot_near, batch_shot_far, cond)
        if self.use_fp16:
            self.optimize_fp16()
        else:
    ...
    def forward_backward(self, batch_shot_near, batch_shot_far, cond):
        zero_grad(self.model_params)

        batch_shot_near = batch_shot_near.to(self.device)
        batch_shot_far = batch_shot_far.to(self.device)

        t, weights = self.schedule_sampler.sample(batch_shot_near.shape[0], self.device)
